...
The reranking helper is implemented as bonus exercise 3.5.
"""
...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        return float(min(max(precision_sum / num_relevant, 0.0), 1.0))
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        rec_list = [r.context_recall for r in results if r.context_recall is not None]
        prec_list = [r.context_precision for r in results if r.context_precision is not None]

        avg_rec = (sum(rec_list) / len(rec_list)) if rec_list else None
        avg_prec = (sum(prec_list) / len(prec_list)) if prec_list else None
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if failure.faithfulness == min_score:
            return "Context is missing or irrelevant — improve retrieval"
        elif failure.relevance == min_score:
            return "Answer does not address the question — improve prompt clarity"
        elif failure.completeness == min_score:
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures, start=1):
            ...
            fix = suggestions[idx - 1] if idx - 1 < len(suggestions) else (suggestions[0] if suggestions else "Investigate pipeline")
    ...
    def generate_improvement_suggestions(
        ...
        if not failures:
            return [
                "Maintain current prompt engineering standards",
                "Monitor retrieval precision on edge cases",
        ...
        if cat.get("irrelevant", 0) > 0 or cat.get("off_topic", 0) > 0:
        ...
        if len(suggestions) < 3:
            suggestions.append("Apply cross-encoder reranking to improve top-k context precision")
        if len(suggestions) < 3:
            suggestions.append("Fine-tune prompt constraints for domain-specific vocabulary")
